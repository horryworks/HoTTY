...
このガイドでは、`themes.json` で利用可能なプロパティについて説明します。HoTTY の外観をカスタマイズするには、ユーザーデータフォルダ（Windowsの場合は `%APPDATA%\HoTTY\themes.json`）にあるこのファイルを編集してください。
...
`themes.json` を変更した後、以下の手順が必要になる場合があります：

...
- **Switch versions from inside HoTTY.** **Settings → Versions** lists every published release, newest first, with the release notes for whichever one you select. Pick a version and HoTTY downloads that installer itself, checks it against the checksum GitHub published for that file, and starts it as HoTTY exits — so nothing is running while files are replaced. Both directions work: upgrade to a newer build, or go back to an older one when a release does not suit you. Going back keeps your hosts, themes and saved credentials; settings that a newer version introduced are ignored rather than deleted. A release GitHub published no checksum for is still listed — its notes are worth reading — but it cannot be installed, because verifying nothing is worse than refusing. Note that these installers are not code-signed, and one downloaded this way does not carry Windows' Mark-of-the-Web, so SmartScreen will not put its usual unknown-publisher prompt in front of it; the checksum check is what stands in for that, which is why a release without one is refused outright.
- **Tab strips scroll.** Terminal tabs, AI Chat conversation tabs and the Settings tabs are now the same scrolling strip. Arrows appear at the ends **only while the row actually overflows**, so a short row looks exactly as it did before. One press moves one tab, holding repeats, and the selected tab is always brought back into view — including when something else selects it, such as a new session or a terminal the AI opened. Arrow keys move the selection and Home/End jump to the ends. Dragging a terminal tab still reorders it, which is why no strip scrolls by dragging: one gesture never has to mean two things.
- **The Settings dialog can be resized.** Drag its edge or corner to make it bigger, and double-click the edge to go back to the default. The size is remembered. The height stays fixed rather than following the active tab — sizing to content made the dialog jump every time you changed tab, and a little empty space is the quieter of the two.

### Improvements

- **Beta builds are told about updates again.** The startup check read GitHub's `/releases/latest`, which filters pre-releases out on the server. A `-betaN` build therefore compared itself against a release feed it could never appear in, and silently reported nothing to install — even when a newer beta, or the stable release that replaced it, had been out for weeks. The check now reads the full releases feed and understands how a pre-release orders against a stable one.
- **The update notification takes you to the update.** It used to send you out to the GitHub release page in a browser. It now opens **Settings → Versions**, where the notes are readable and the switch is actually performed, so there is nothing left for a browser to do. Stepping aside this way is not recorded as dismissing it — the notification returns on the next launch if you did not install anything.

### Security

- **The version switcher cannot be pointed anywhere except GitHub.** The UI never handles a download URL: it names a release tag, and HoTTY resolves that tag against the release list it fetched itself, so there is no field a crafted value could travel through. The download host is pinned to this project's own path on `github.com` over HTTPS, and every redirect hop is checked too — a redirect toward any other host stops the download rather than following it. The installer's file name is validated before anything is written to disk, and the downloaded bytes are hashed and compared with GitHub's published SHA-256 for that asset before the file can be launched at all. At most one installer image is ever kept on disk, and it is cleared on the next start.

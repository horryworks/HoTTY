...
export const help = {
  ...
  gettingStarted: {
    ...
    gcpSshUser:
      '<0>Usuario SSH (normalmente en blanco):</0> HoTTY determina con qué cuenta de Linux iniciar sesión consultando a gcloud, así que el campo <1>SSH user</1> de la pestaña GCP normalmente se deja vacío. Rellénalo solo si una VM acepta una cuenta distinta de la detectada: la conexión falla entonces con <2>Permission denied (publickey)</2> y el error indica la cuenta que se intentó. Ten en cuenta que HoTTY lee la respuesta de gcloud sin modificar nada en el lado de Google, por lo que en un equipo cuya clave SSH nunca se registró aún debes ejecutar <3>gcloud compute ssh &lt;instance&gt; --tunnel-through-iap</3> una vez para registrarla.',
    gcpFiltering:

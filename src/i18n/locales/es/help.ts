...
export const help = {
  ...
  gettingStarted: {
    ...
    updateNotifications:
      '<0>Notificaciones de actualización:</0> Al iniciarse, HoTTY consulta el feed de versiones de GitHub y muestra una notificación descartable cuando hay una versión más reciente. Se incluyen las versiones preliminares, por lo que una beta recibe aviso tanto de betas más nuevas como de la versión estable que la sustituye. La notificación abre Ajustes → Versiones, donde se aplica la actualización.',
    versionSwitching:
      '<0>Cambiar de versión:</0> Ajustes → Versiones enumera todas las versiones publicadas con sus notas e instala la que elijas, hacia una compilación más nueva o de vuelta a una anterior. HoTTY descarga el instalador por su cuenta, lo coteja con la suma de comprobación que GitHub publicó para ese archivo y lo inicia al salir HoTTY, de modo que nada esté en ejecución mientras se reemplazan los archivos. Volver atrás conserva tus hosts, temas y credenciales guardadas; los ajustes añadidos por una versión más nueva se ignoran, no se eliminan. Una versión para la que GitHub no publicó suma de comprobación aparece en la lista pero no se puede instalar.',
    connectionStatus:
  ...
  aiFeatures: {
    ...
    interactiveBody:
      'Cuando la IA sugiere un comando, puede enviarlo directamente a su sesión de terminal para ejecutarlo. Verá el comando antes de que se ejecute, por lo que mantiene el control. Cuando un comando sugerido está a la espera de su confirmación, puede ejecutarlo o hacer clic en «No ejecutar» para rechazarlo; al rechazarlo, la IA se entera y puede sugerir un enfoque diferente.',
    ...
    safetyBody:
      'Cómo se juzga cada comando para la ejecución automática lo deciden tres capas, configurables en <0>Configuración → IA → Ejecución de comandos</0>. La <1>lista negra</1> se comprueba primero — una coincidencia nunca se ejecuta automáticamente (todavía se ofrece una ejecución manual). Por encima de ambas listas hay dos límites que no se pueden editar: un comando que redirige, encadena o sustituye nunca se ejecuta automáticamente, y tampoco uno que pueda enviar datos fuera de este equipo (curl, wget, nc, nmap y similares); esos siempre preguntan, sea cual sea la estrategia elegida. La <2>lista blanca</2> ejecuta automáticamente comandos obvios de solo lectura. Cualquier cosa intermedia se envía a la <3>IA</3>, que juzga si cambia la configuración; solo los comandos juzgados como de solo lectura con suficiente confianza se ejecutan automáticamente, todo lo demás pregunta. Ambas listas son totalmente editables: una sola palabra coincide con un comando base (p. ej. <4>docker</4>), y una entrada con espacios coincide como subcadena (p. ej. <5>rm -rf</5>, <6>git push</6>); cada lista tiene un botón de <7>Restablecer valores predeterminados</7>. Elija la estrategia (Estático / IA / <8>Híbrido</8>, el predeterminado) y el umbral de confianza de la IA en el mismo lugar. Cada bloque de ejecución lleva una línea de veredicto con código de color — Seguro · lista blanca, Seguro · IA, Revisar · IA, Bloqueado o Necesita confirmación — y al hacer clic se despliega el motivo completo, de modo que una ejecución segura habitual se lee sin ruido y la decisión queda siempre a un clic.',
    ...
    connectHeading: 'Terminales abiertos por la IA',
    connectBody:
      'Cuando una investigación lo requiere, la IA puede pedir a HoTTY que le abra un terminal: una <0>shell del PC</0> (para ejecutar ping, tracert o nslookup desde su equipo) o una <1>sesión SSH/Telnet a un vecino</1> descubierto con CDP/LLDP. La solicitud aparece como una tarjeta en el chat con el destino, el usuario y el origen de las credenciales; ahí la aprueba o la rechaza. Las sesiones abiertas por la IA no tienen pestaña (su salida se captura para la IA y aparece en la conversación) y se muestran como chips junto a los terminales monitorizados, donde puede abrir una como pestaña real o desconectarla. Están limitadas por conversación, se cierran automáticamente cuando quedan inactivas y se cierran al cerrar la conversación. Configure la política, la shell, el límite y el tiempo de inactividad en <2>Ajustes → IA</2>; reutilizar la contraseña de un terminal monitorizado está desactivado por defecto y siempre pregunta antes. La comprobación de la clave de host SSH no cambia: la primera conexión a un dispositivo desconocido sigue pidiéndole confirmación.',
  },
  ...
  themes: {
    ...
    language:
      '<0>Idioma de la interfaz:</0> cambie el idioma de la interfaz en <1>Configuración → General → Idioma de la interfaz</1>. HoTTY está disponible en English, 日本語, 简体中文, 繁體中文, 한국어, Русский, Español y Français; el cambio se aplica al instante. (El idioma de respuesta de la IA sigue esta configuración de forma predeterminada; puede anularlo en los ajustes del panel de chat de IA.)',
  },
};

// 設定強制回應視窗 — 外框、分頁標籤，以及所有分頁內容。
export const settings = {
  title: '設定',
  tabs: {
    general: '一般',
    appearance: '外觀',
    protocols: '通訊協定',
    sshKeys: 'SSH 金鑰',
    features: '功能',
    netbox: 'NetBox',
    ai: 'AI',
    versions: '版本',
    about: '關於',
  },
  general: {
    // 語言選擇器（此步驟）
    languageSection: '語言',
    languageLabel: '顯示語言',
    languageHelp:
      '變更應用程式介面的語言。（AI 回應語言預設跟隨此設定，也可在 AI 聊天面板中另行指定。）',
    // 記錄
    loggingSection: '記錄',
    enableLogging: '啟用記錄',
    logFolderPath: '記錄檔資料夾路徑',
    logFolderPathHelp: '終端機記錄檔會以 YYYYMMDDHHMMSS-(Protocol)-(IP).txt 格式儲存，AI 聊天記錄則以 YYYYMMDDHHMMSS-AICHAT-(聊天).md 格式儲存',
    logFolderPathPlaceholder: '選取資料夾或輸入路徑...',
    // 終端機
    terminalSection: '終端機',
    scrollbackBuffer: '回捲緩衝區',
    scrollbackHelp: '每個終端機保留在記憶體中的最大行數（預設值：10000）。',
    enableLineWrap: '啟用自動換行',
    fixedTerminalSizeMode: '固定終端機尺寸',
    fixedTerminalSizeModeHelp: '將終端機寬度固定為連線時協商的尺寸，而不在視窗調整時重新排版。登入時鎖定寬度並忽略後續調整的裝置（如華為 USG/VRP）需要此項——重新排版會使其行編輯錯位。「自動」僅在 SSH 伺服器被識別為此類裝置時才固定。可依連線個別覆寫。',
    fixedTerminalSizeModeOff: '關閉 — 從不固定',
    fixedTerminalSizeModeAuto: '自動 — 偵測裝置後固定（建議）',
    fixedTerminalSizeModeOn: '開啟 — 一律固定',
    // 輸入
    inputSection: '輸入',
    backspaceSendsDel: 'Backspace 傳送 DEL（0x7F）',
    backspaceSendsDelHelp:
      '若停用，Backspace 會傳送 0x08（BS）。若您的伺服器預期收到 0x7F，請啟用此選項。',
    rightClickPaste: '按右鍵以貼上',
    rightClickPasteHelp: '在終端機上按右鍵會顯示貼上確認對話方塊。',
    // 診斷
    diagnosticsSection: '診斷',
    debugLog: '偵錯記錄',
    debugLogHelp: '回報錯誤時請分享最新的記錄檔。',
    openDebugLogFolder: '開啟偵錯記錄資料夾',
  },
  appearance: {
    // 版面配置
    layoutSection: '版面配置',
    sidebarPosition: '側邊欄位置',
    sidebarLeft: '左側',
    sidebarRight: '右側',
    // 佈景主題
    themeSection: '佈景主題',
    themeLabel: '佈景主題',
    deleteTheme: '刪除',
    createCustomTheme: '建立自訂佈景主題',
    deleteThemeTitle: '刪除佈景主題',
    deleteThemeMessage:
      '要刪除自訂佈景主題「{{name}}」嗎？此動作無法復原。',
    // 未使用窗格的背景
    unusedPaneBackground: '未使用窗格的背景',
    paneBackgroundColor: '色彩',
    paneBackgroundImage: '影像',
    paneBackgroundImagePlaceholder: '例如 http://asset.localhost/...',
    browse: '瀏覽…',
    clearImage: '清除影像',
    clear: '清除',
    // 字型
    fontSection: '字型',
    fontFamily: '字型系列',
    systemMonospaceDefault: '系統等寬字型（預設）',
    scanning: '正在掃描...',
    rescan: '重新掃描',
    fontSize: '字型大小（px）',
    webBrowserSection: 'Web 瀏覽器',
    webBrowserDefaultZoom: '預設縮放（%）',
    webBrowserDefaultZoomHelp: '每個新開啟的 Web 瀏覽器窗格的初始縮放比例（25–500%）。之後每個窗格在工作階段期間保留各自的縮放比例。',
    // 終端機顯示
    terminalDisplaySection: '終端機顯示',
    defaultEncoding: '預設編碼',
    defaultEncodingHelp: '套用於新連線。',
    promptHighlight: '提示詞醒目顯示',
    enableUserInputHighlight: '啟用使用者輸入醒目顯示',
    highlightColor: '醒目顯示色彩',
    promptPatternsRegex: '提示詞模式（規則運算式）',
    namePlaceholder: '名稱',
    regexPlaceholder: '規則運算式',
    addPattern: '+ 新增模式',
    resetToDefault: '重設為預設值',
  },
  protocols: {
    // SSH
    sshSection: 'SSH',
    connectTimeout: '連線逾時',
    timeoutSeconds: '逾時（秒）',
    sshTimeoutHelp:
      '放棄前等待初始 TCP 與 SSH 交握的最長時間。預設值：5 秒。',
    keepAlive: 'KeepAlive',
    enable: '啟用',
    sshKeepAliveHelp: '傳送虛擬封包以防止逾時。',
    intervalSeconds: '間隔（秒）',
    // 演算法
    algorithms: '演算法',
    algorithmsHelp: '選擇要啟用哪些演算法。變更會套用至新工作階段。',
    categoryServerHostKey: '伺服器主機金鑰',
    categoryKex: '金鑰交換',
    categoryCipher: '加密演算法',
    categoryMac: 'MAC',
    // Telnet
    telnetSection: 'Telnet',
    telnetTimeoutHelp:
      '放棄前等待初始 TCP 連線的最長時間。預設值：5 秒。',
    telnetKeepAliveHelp: '傳送 Telnet NOP 指令以防止閒置逾時。',
    // DH-GEX SHA-1 警告
    dhGexTitle: '要啟用 diffie-hellman-group-exchange-sha1 嗎？',
    dhGexConfirm: '啟用',
    dhGexWarning:
      'diffie-hellman-group-exchange-sha1 依賴 SHA-1，而 SHA-1 已被視為' +
      '不安全，並已於 OpenSSH 8.2 從預設值中移除。僅在您必須' +
      '與沒有提供更強金鑰交換的舊式裝置通訊時才啟用它。\n\n' +
      '只要遠端裝置支援，請優先使用 diffie-hellman-group-exchange-sha256、' +
      'diffie-hellman-group14-sha256 或 curve25519-sha256。\n\n' +
      '仍要啟用 diffie-hellman-group-exchange-sha1 嗎？',
  },
  sshKeys: {
    generateSection: '產生新金鑰',
    generateButton: '產生金鑰…',
    generateHelp:
      '金鑰會以一般 OpenSSH 檔案的形式存放在 .ssh 資料夾中，因此 PuTTY、WinSCP 和 ssh 指令也能使用。HoTTY 會為每個新金鑰收緊權限，只有你本人可以讀取。',
    keysSection: '.ssh 資料夾中的金鑰',
    openFolder: '開啟資料夾',
    refresh: '重新整理',
    empty: '這裡還沒有金鑰。',
    loadError: '無法讀取 .ssh 資料夾。',
    truncated: '此資料夾檔案較多，僅顯示前 {{count}} 個。',
    createdByHotty: '由 HoTTY 建立',
    hasPassphrase: '有密碼短語',
    noPublicFile: '沒有 .pub 檔案',
    unreadable: '無法讀取此金鑰。',
    createdAt: '建立於 {{date}}',
    copyPublicKey: '複製公開金鑰',
    exportPublicKey: '儲存公開金鑰…',
    showAuthorizedKeys: '顯示要貼到伺服器的那一行',
    authorizedKeysHelp: '把這一行加到目標伺服器的 ~/.ssh/authorized_keys 中。',
    deleteKey: '刪除',
    deleteOnlyOwn: '這裡只能刪除 HoTTY 建立的金鑰。其他金鑰請到資料夾中刪除。',
    deleteConfirmTitle: '要刪除這個金鑰嗎？',
    deleteConfirmMessage:
      '將刪除 {{name}} 及其公開金鑰。使用它的已儲存連線將無法運作。此操作無法復原。',
    deleteFailed: '無法刪除金鑰。',
    modalTitle: '產生 SSH 金鑰',
    nameLabel: '檔案名稱',
    nameHelp: '此金鑰存放在 .ssh 資料夾中所使用的檔案名稱。',
    algorithmLabel: '類型',
    commentLabel: '備註',
    commentHelp: '會隨公開金鑰一起出現在伺服器上，通常填電子郵件地址。',
    passphraseLabel: '密碼短語（選填）',
    passphraseConfirmLabel: '確認密碼短語',
    passphraseHelp: '保護金鑰檔案本身，即使檔案被竊取也無法使用。連線時會要求輸入。',
    passphraseMismatch: '兩次輸入的密碼短語不一致。',
    noPassphraseNote: '若不設定密碼短語，取得檔案的人都能使用這個金鑰。',
    generate: '產生',
    generating: '產生中…',
    generatingSlow: '產生 RSA 金鑰需要幾秒鐘，請稍候。',
    generateFailed: '無法產生金鑰。',
    createdTitle: '金鑰已建立',
    createdHelp: '複製這一行並加到伺服器。',
    createdLaterHint: '之後可以在「設定 ▸ SSH 金鑰」中再次查看這一行。',
    useForConnection: '本次連線使用此金鑰',
    algoEd25519: 'Ed25519（建議）',
    algoEcdsaP256: 'ECDSA P-256',
    algoEcdsaP384: 'ECDSA P-384',
    algoEcdsaP521: 'ECDSA P-521',
    algoRsa2048: 'RSA 2048',
    algoRsa3072: 'RSA 3072',
    algoRsa4096: 'RSA 4096',
    algoHelpEd25519: '體積小、速度快，較新的設備基本都支援。',
    algoHelpEcdsa: '當設備不接受 Ed25519 時使用。',
    algoHelpRsa: '用於較舊的網路設備。若設備只接受舊的 ssh-rsa 簽章，還需要在「通訊協定」中啟用它。',
    nameErrorEmpty: '請輸入名稱。',
    nameErrorTooLong: '名稱太長。',
    nameErrorInvalidCharacters: '只能使用字母、數字、點、底線和連字號。',
    nameErrorStartsWithDot: '名稱不能以點開頭。',
    nameErrorEndsWithDot: '名稱不能以點結尾。',
    nameErrorIsPublicKey: '以 .pub 結尾的名稱屬於公開金鑰。',
    nameErrorReservedDevice: '該名稱已被 Windows 保留。',
    nameErrorTaken: '同名的金鑰已存在。',
    commentErrorTooLong: '備註太長。',
    commentErrorHasLineBreak: '備註不能包含換行。',
  },
  features: {
    fileServerLabel: '檔案伺服器',
    fileServerDescription: '用於向網路裝置上傳韌體的 TFTP / SFTP 伺服器',
    webBrowserLabel: '網頁瀏覽器',
    webBrowserDescription: '用於網路裝置網頁管理介面的內嵌瀏覽器窗格',
    section: '功能',
    sectionHelp: '啟用或停用功能窗格。已開啟的窗格不受影響。',
    aiChatLabel: 'AI 聊天',
    aiChatDescription: '由 AI 驅動的聊天面板，用於協助終端機操作',
    logViewerLabel: '記錄檢視器',
    logViewerDescription: '檢視並分析工作階段記錄',
    pingMonitorLabel: 'Ping 監控',
    pingMonitorDescription: '持續 ICMP Ping 監控',

    interfaceTrafficLabel: '介面流量',

    interfaceTrafficDescription: '透過 SNMP 即時檢視網路裝置的介面計數器（bps / pps / 錯誤）',
  },
  netbox: {
    serverSection: 'NetBox 伺服器',
    baseUrl: '基礎 URL',
    baseUrlHelp: 'NetBox 的位址，例如 https://netbox.example.com。HoTTY 只會讀取，絕不寫回。',
    baseUrlPlaceholder: 'https://netbox.example.com',
    insecureWarning: '此位址使用 http://，API 權杖將以明文傳送。',
    token: 'API 權杖',
    tokenHelp: '具有 Regions 與 Sites 讀取權限的 NetBox API 權杖。以 Windows DPAPI 加密保存，不會離開本機。',
    tokenPlaceholder: '貼上權杖',
    tokenSaved: '已儲存權杖。',
    replaceToken: '更換權杖',
    connect: '連線',
    connecting: '連線中…',
    disconnect: '中斷連線',
    disconnectHelp: '忘記權杖。已同步的資料夾會保留在樹狀結構中，只是不再更新。',
    resultConnected: '已連線到 NetBox {{version}}。',
    resultConnectedNoVersion: '已連線到 NetBox。',
    resultRefused: '此位址確實是 NetBox（API {{apiVersion}}），但權杖被拒絕。',
    resultRefusedNoVersion: '此位址有回應，但權杖被拒絕。',
    resultNotNetbox: '此位址有回應，但不是 NetBox API（HTTP {{status}}）。',
    resultUnreachable: '無法在此位址存取 NetBox API。',
    siteIdSection: 'Site ID 欄位',
    siteIdLabel: '哪個 NetBox 欄位存放站點代碼',
    siteIdHelp: '站點資料夾會命名為「代碼 站點名稱」。代碼放在哪個欄位因組織而異：NetBox 內建的 facility 欄位常常是空的，代碼多半放在自訂欄位中。',
    siteIdNone: '（不加代碼 — 僅使用站點名稱）',
    siteIdOther: '其他 — 自行輸入欄位名稱…',
    siteIdCustomKeyPlaceholder: '自訂欄位名稱',
    siteIdCustomKeyInvalid: '自訂欄位名稱只能包含字母、數字與底線。',
    siteIdExample: '資料夾將命名為「{{example}}」。',
    customFieldsDenied: 'HoTTY 無法讀取此 NetBox 的自訂欄位定義：API 權杖缺少 extras.view_customfield 權限。若站點代碼位於自訂欄位，請選擇「其他」並輸入欄位名稱。',
    customFieldsEmpty: '此 NetBox 未定義可存放站點代碼的自訂欄位。',
    refreshFields: '重新整理欄位清單',
    syncSection: '同步',
    syncOnStartup: 'HoTTY 啟動時同步',
    syncOnStartupHelp: '僅在主視窗於啟動時執行一次。失敗不會彈出視窗，而是記錄在此處。',
    syncNow: '立即同步',
    syncing: '同步中…',
    notConfigured: '請在上方填寫 NetBox 位址與權杖以啟用同步。',
    neverSynced: '尚未同步。',
    lastSync: '上次同步：{{when}}',
    lastError: '上次同步失敗：{{message}}',
    summaryCounts: '已同步 {{regions}} 個 region、{{sites}} 個 site。',
    summaryMissing: '已保留但 NetBox 中不存在的資料夾：{{count}} 個。',
    summaryDetached: '被你移出 NetBox 資料夾的：{{count}} 個。仍會更名，但不會重新排列。',
    summaryDuplicates: '來自匯入樹狀結構的重複 NetBox 資料夾：{{count}} 個，現已成為一般資料夾。',
    summarySiteIdPartial: '{{sites}} 個站點中有 {{without}} 個沒有 Site ID 值。',
    summarySiteIdNone: '{{sites}} 個站點全部沒有 Site ID 值，資料夾保留了 NetBox 的原始名稱。請換一個欄位試試。',
    placementSection: '依 IP 歸置主機',
    placementToggle: '用 NetBox 的 IP 網段決定儲存資料夾',
    placementHelp: '新增主機時，HoTTY 會把它的 IP 位址拿去比對掛在區域與站台上的 IPAM 前綴，並預先選取相符的資料夾。這只是建議：儲存前你可以改選別的資料夾，同步也絕不會自行移動主機。',
    placementFetchNote: '關閉期間完全不會取得前綴。開啟後請按「立即同步」。',
    placementScan: '依 IP 網段歸類所有主機…',
    placementScanHelp: '將節點樹中的所有主機與 IP 網段比對，並顯示哪些會移動。確認之前不會移動任何內容，也可以先取消勾選某些列。',
    placementScanMoved_one: '已移動 {{count}} 台主機。',
    placementScanMoved_other: '已移動 {{count}} 台主機。',
    summaryPrefixes: '讀取了 {{count}} 筆前綴；現在有 {{folders}} 個資料夾可以比對主機。',
    summaryPrefixesNoFolders: '讀取了 {{count}} 筆前綴，但沒有任何一筆落到資料夾上——目前不會有任何相符。',
    summaryPrefixesSkipped: '忽略了 {{count}} 筆前綴，它們掛在 HoTTY 沒有對應資料夾的物件上（位置、站台群組，或沒有歸屬）。',
    summaryPrefixesUnparsed: '有 {{count}} 筆前綴無法以 IP 網段讀取。',
    prefixesDenied: 'API 權杖無權讀取前綴（ipam.view_prefix）。已儲存的前綴會保留，歸置仍會依據它們運作。',
    prefixesTooMany: '此 NetBox 的前綴數量超出 HoTTY 的讀取上限。已儲存的前綴會保留；區域與站台已正常同步。',
    prefixesDisabled: '依 IP 歸置主機已關閉，因此沒有請求前綴。',
  },
  ai: {
    // 供應商
    providerSection: '供應商',
    aiProvider: 'AI 供應商',
    aiProviderHelp:
      '選擇供應商後在下方登入。Gemini 使用 Google 登入（OAuth），Vertex AI 使用您的 Google Cloud 專案（ADC 或服務帳戶金鑰），Anthropic 與 OpenAI 使用 API 金鑰。',
    providerVertexAi: 'Google Cloud Vertex AI',
    providerGemini: 'Google AI Studio（Gemini）',
    providerAnthropic: 'Anthropic（Claude）',
    providerOpenai: 'OpenAI',
    // 驗證
    authentication: '驗證',
    authenticated: '已驗證',
    notAuthenticated: '尚未驗證',
    logout: '登出',
    // 憑證輸入表單（依供應商）
    auth: {
      // Gemini（Google AI Studio）
      geminiTitle: '連線至 Gemini',
      clientId: '用戶端 ID',
      clientSecret: '用戶端密碼',
      connecting: '正在連線...',
      signInWithGoogle: '使用 Google 登入',
      // Vertex AI
      vertexTitle: '連線至 Vertex AI',
      gcpProjectId: 'GCP 專案 ID',
      gcpProjectIdPlaceholder: 'my-project-id',
      location: '位置',
      locationPlaceholder: 'us-central1',
      authMethod: '驗證方法',
      authMethodAdc: '應用程式預設憑證（ADC）',
      authMethodServiceAccount: '服務帳戶金鑰檔案',
      serviceAccountKeyFile: '服務帳戶金鑰檔案',
      serviceAccountKeyFilePlaceholder: '/path/to/service-account-key.json',
      browse: '瀏覽...',
      connectVertex: '連線至 Vertex AI',
      // OpenAI
      openaiTitle: '連線至 OpenAI',
      connectOpenai: '連線至 OpenAI',
      openaiKeyPlaceholder: 'sk-...',
      // Anthropic
      anthropicTitle: '連線至 Anthropic',
      connectAnthropic: '連線至 Anthropic',
      anthropicKeyPlaceholder: 'sk-ant-...',
      // 共用
      apiKey: 'API 金鑰',
      failed: '驗證失敗。請再試一次。',
      timedOut: '驗證逾時。請再試一次。',
    },
    // 角色
    personasSection: '角色',
    addPersona: '新增角色',
    personaName: '角色名稱',
    displayNamePlaceholder: '顯示名稱',
    systemPrompt: '系統提示詞',
    systemPromptPlaceholder: '系統提示詞',
    deletePersona: '刪除角色',
    atLeastOnePersona: '至少需要一個角色',
    deletePersonaTitle: '刪除「{{label}}」',
    resetAllPersonas: '重設所有角色',
    newPersonaLabel: '新角色',
    // 指令執行
    commandExecutionSection: '指令執行',
    commandExecutionHelp:
      '執行模式與自動執行上限在 AI 聊天窗格中設定（位於訊息輸入區下方）。',
    commandSafetyClassifier: '指令安全性分類器',
    commandSafetyClassifierHelp:
      'HoTTY 如何判斷是否自動執行 AI 建議的指令。黑名單一律最先檢查（命中時會先詢問再執行）。靜態：白名單自動執行，其餘皆詢問。AI：由 AI 判斷任何未列入黑名單的指令。混合（建議）：白名單自動執行，其餘交由 AI 判斷，否則詢問。',
    classifierStatic: '靜態白名單',
    classifierAi: 'AI 判斷',
    classifierHybrid: '混合（建議）',
    aiConfidenceThreshold: 'AI 信心門檻：{{percent}}%',
    aiConfidenceThresholdHelp:
      '自動執行被判定為唯讀的指令所需的最低 AI 信心度。低於此值時，指令會等待手動執行。值越高越謹慎。預設值：70%。',
    autoExecCountdown: '自動執行倒數（秒）',
    autoExecCountdownHelp:
      '在「安全自動執行」模式下，判定為安全的指令會在等待這些秒數後再執行，讓你能及時取消。0 表示立即執行。最大 10。預設：3。',
    concurrentStreams: 'AI 聊天並行回覆數',
    concurrentStreamsHelp:
      '一個窗格中可同時接收回覆的 AI 聊天分頁數量。超出的傳送會排入佇列，並在回覆完成後開始。1 表示嚴格一次一個（舊行為）。若你的供應商有速率限制（如 Gemini 免費層），請調低。最大 8。預設：3。',
    connectPolicy: '允許 AI 開啟終端機',
    connectPolicyHelp:
      'AI 可以要求 HoTTY 開啟本機 Shell（用於執行 ping / tracert / nslookup），或開啟到透過 CDP/LLDP 發現的鄰接裝置的 SSH/Telnet 工作階段。這些工作階段沒有分頁（輸出為 AI 擷取），可隨時以分頁開啟。關閉：忽略要求。總是詢問：每個要求都需要在聊天中核准。本機 Shell 自動：本機 Shell 不詢問即開啟，裝置登入仍需確認。本機 + 主機樹自動：還會使用已儲存的認證資訊開啟主機與主機樹項目相符的 SSH/Telnet。自動開啟僅在自動執行模式下生效，並使用自動執行倒數。預設：一律詢問。',
    connectPolicyOff: '關閉',
    connectPolicyAsk: '總是詢問',
    connectPolicyLocalAuto: '本機 Shell 自動，裝置需確認',
    connectPolicyHostTreeAuto: '本機 Shell + 主機樹裝置自動',
    connectLocalShell: 'AI 使用的本機 Shell',
    connectLocalShellHelp:
      'AI 要求本機終端機時 HoTTY 開啟的 Shell。每個對話一個，再次要求時會重複使用已開啟的 Shell。',
    connectMaxPerTab: '每個對話中 AI 開啟終端機的上限',
    connectMaxPerTabHelp:
      '一個對話中 AI 可同時保持開啟的終端機數（包括已轉為分頁的）。保護裝置有限的 VTY 線路。關閉一個即可釋出名額。1–10。預設：5。',
    connectIdleTimeout: '閒置的 AI 終端機關閉時間（分鐘）',
    connectIdleTimeoutHelp:
      '沒有分頁且 AI 在此時間內未使用的 AI 開啟終端機將自動中斷連線。已轉為分頁的終端機屬於您，將保持開啟。0 = 永不。預設：10。',
    connectReuseCredentials: '允許沿用監看中終端機的認證資訊',
    connectReuseCredentialsHelp:
      '當 AI 在要求中指定監看中的終端機（via: <別名>）時，會將該工作階段的登入名稱與密碼複製到新的 SSH/Telnet 登入。始終要求您確認，並說明認證資訊來自哪個終端機。關閉：僅沿用登入名稱。預設關閉，因為偽造的 CDP/LLDP 鄰接資訊可能把 AI 引向惡意主機。',
    deviceResponseTimeout: '裝置回應逾時（秒）',
    deviceResponseTimeoutHelp:
      '若裝置在指令後的這麼多秒內未產生新輸出，系統會告知 AI 裝置已停止回應，以便迴圈得以繼續。0 會停用閒置偵測。預設值：10。',
    sleepAsClientDelay: '將開頭的 `sleep` 作為用戶端延遲執行',
    sleepAsClientDelayHelp:
      '當 AI 發出以 `sleep N` 開頭的指令（例如 `sleep 120 && validate`）時，在 HoTTY 中等待 N 秒，而非在裝置上執行 sleep。這可避免上述的裝置回應逾時在 sleep 期間誤觸發。任何串接的指令會在等待後執行。預設值：開啟。',
    maxClientDelay: '最大用戶端延遲（秒）',
    maxClientDelayHelp:
      '用戶端 sleep 延遲的上限；較長的 sleep 會被限制至此值並加註說明。0 = 無上限。預設值：900（15 分鐘）。',
    whitelist: '白名單（自動執行）',
    whitelistHelp:
      '在此處比對到的指令會自動執行。單一字詞會比對為基礎指令（例如「docker」可比對任何 docker 指令）；含空格的項目則會比對為指令前綴（例如「git log」）。已預先填入安全的預設值；可完整編輯。',
    whitelistPlaceholder: '例如 docker、kubectl get',
    add: '新增',
    resetToDefaults: '重設為預設值',
    resetWhitelistTitle: '將白名單重設為內建預設值',
    removeEntry: '移除 {{cmd}}',
    blacklist: '黑名單（執行前先詢問）',
    blacklistHelp:
      '在此處比對到的指令永遠不會自動執行 — 仍可手動執行，但會顯示警告。單一字詞會比對為基礎指令；含空格的項目則會比對為子字串（例如「rm -rf」、「git push」）。已預先填入具破壞性的預設值；可完整編輯。',
    blacklistPlaceholder: '例如 rm -rf、git push',
    resetBlacklistTitle: '將黑名單重設為內建預設值',
    // 強制回應視窗
    privacyNoticeTitle: '隱私權聲明',
    privacyNoticeMessage:
      'Google AI Studio（Gemini）在免費方案中可能會使用您的資料進行 AI 訓練。請在您的 Google Cloud 專案上啟用計費以選擇退出。',
    privacyNoticeConfirm: '確定',
    logoutTitle: '登出',
    logoutMessage:
      '確定要登出嗎？您將需要重新驗證才能使用 AI 供應商。',
    logoutConfirm: '登出',
    // 資料處理告知
    dataHandlingSection: '資料處理',
    dataHandlingHelp:
      '當您使用 AI 功能時，系統會使用您自己的 API 金鑰，將資料傳送給您設定的第三方 AI 供應商，並受該供應商的條款與隱私權政策約束。',
    dataHandlingBulletProviders:
      '供應商：Google Gemini / Vertex AI、Anthropic 或 OpenAI——以您在上方選取的為準。',
    dataHandlingBulletWhen:
      '僅在您明確使用 AI 功能（聊天、詢問 AI 或監看）時傳送。您的終端機絕不會被持續傳輸。',
    dataHandlingBulletRedaction:
      '已知的密鑰模式會從記錄中隱去，但您在訊息中輸入的文字會原樣傳送——請勿貼上憑證。',
    dataConsentStatus: '告知同意',
    dataConsentAccepted: '已接受',
    dataConsentNotAccepted: '尚未顯示',
    resetDataConsent: '重新顯示',
    resetDataConsentHelp:
      '重設同意，以便在下次 AI 傳送前再次顯示 AI 資料分享告知。',
  },
  versions: {
    title: 'HoTTY 版本',
    installedLabel: '目前使用: v{{version}}',
    showPrereleases: '顯示測試版',
    refresh: '重新整理',
    loading: '正在載入版本清單…',
    empty: '找不到任何版本',
    selectPrompt: '選擇一個版本以檢視發行說明',
    noNotes: '此版本沒有發行說明',
    badgeCurrent: '使用中',
    badgePrerelease: '測試版',
    notInstallable: '未發布校驗碼，無法驗證此版本',
    install: '切換至 v{{version}}',
    upgrade: '升級',
    downgrade: '降級',
    installing: '正在安裝…',
    cancel: '取消',
    downloading: '正在下載… {{percent}}%',
    verifying: '正在驗證校驗碼…',
    launching: '正在啟動安裝程式…',
    downgradeNote: '回到舊版本後，主機清單、佈景主題與已儲存的憑證都會保留。新版本新增的設定會被忽略，但不會被刪除。',
    failed: '切換版本失敗: {{message}}',
  },
  about: {
    version: 'v{{version}}',
    author: 'Katsumasa "Horry" Horiuchi',
    descriptionLine1: 'SSH/Telnet/序列埠終端機模擬器',
    descriptionLine2: '以 Tauri、React 與 TypeScript 建構',
    licenseLine1: '本程式為自由軟體，依據',
    licenseLine2: 'GNU General Public License v3.0 或更新版本發行。',
    viewLicense: '檢視 GNU General Public License v3.0',
    logoAlt: 'HoTTY 標誌',
    // 第三方授權
    thirdPartyLicenses: '第三方授權',
    thirdPartyLicensesTitle: '第三方授權',
    thirdPartyLicensesIntro: 'HoTTY 包含來自以下專案的開放原始碼軟體：',
    thirdPartyLicensesLoading: '正在載入授權…',
    thirdPartyLicensesError: '載入授權資訊失敗。',
    thirdPartyLicensesEmpty: '沒有可用的第三方授權資訊。',
    thirdPartyLicensesClose: '關閉',
  },
  customTheme: {
    // Tooltip text for each themeable CSS variable, keyed by its camelCased
    // variable name. Rendered as the `title` of the variable row, so these are
    // user-facing strings and belong here rather than inline in the component.
    vars: {
      bgPrimary: '應用程式與作用中窗格的主背景色。',
      bgSecondary: '側邊欄、標題列與介面元素的背景色。',
      bgTertiary: '非作用中索引標籤與下拉選單的背景色。',
      textPrimary: '主要文字色彩。',
      textSecondary: '次要文字或提示的色彩。',
      textTertiary: '最暗一級文字的色彩（如拖曳控點、淡化提示）。',
      textOnAccent: '疊加在強調色元素上的文字色彩（如按鈕）。',
      borderColor: '窗格框線與分隔線的色彩。',
      accentColor: '按鈕、作用中指示器與連結的主要色彩。',
      accentHover: '強調元素的暫留色彩。',
      accentLight: '強調色的較亮變化色。',
      accentSecondary: '用於額外醒目提示的次強調色。',
      linkColor: '超連結與 URL 參照的色彩。',
      inputBg: '文字輸入欄與設定列的背景色。',
      btnBg: '標準按鈕的背景色。',
      btnHoverBg: '標準按鈕暫留時的背景色。',
      btnSecondaryBg: '次要按鈕（如取消）的背景色。',
      btnSecondaryHoverBg: '次要按鈕暫留時的背景色。',
      btnDangerBg: '破壞性操作按鈕的背景色。',
      btnDangerHoverBg: '危險按鈕暫留時的背景色。',
      hoverBg: '清單項目或可互動列的一般暫留背景色。',
      placeholderColor: '輸入欄中預留位置文字的色彩。',
      successColor: '成功指示（如連線建立）使用的色彩。',
      statusSuccess: '已驗證/成功狀態指示點的色彩。',
      statusError: '未驗證/錯誤狀態指示點的色彩。',
      colorDanger: '表示破壞性操作或錯誤的色彩。',
      colorDangerShade: '危險色的較暗變化色，用於隱藏 AI 索引標籤漸層的兩端。',
      colorDangerBg: '危險元素的背景色調。',
      colorDangerBgHover: '危險元素暫留時的背景色調。',
      colorDangerBorder: '危險元素的框線色彩。',
      colorWarning: '警告或需要注意的文字色彩。',
      chatMsgUserBg: '使用者傳送訊息的背景色。',
      chatMsgUserText: '使用者訊息的文字色彩。',
      chatMsgModelText: 'AI 回覆的文字色彩。',
      codeBg: '交談中程式碼區塊的背景色。',
      codeText: '程式碼區塊的文字色彩。',
      aiHeaderBg: 'AI 交談標題列的背景色調。',
      aiWelcomeText: '大型歡迎標題的色彩。',
      aiWelcomeSubtext: '交談為空時說明文字的色彩。',
      aiChatContentFontSize: 'AI 交談訊息內容的字型大小（如 1.1em）。',
      aiTokenFontSize: '權杖用量狀態列的字型大小（如 0.78em）。',
      aiMarkdownH1FontSize: 'AI Markdown 中 h1 標題的字型大小（如 1.3em）。',
      aiMarkdownH2FontSize: 'AI Markdown 中 h2 標題的字型大小（如 1.15em）。',
      aiMarkdownH3FontSize: 'AI Markdown 中 h3 標題的字型大小（如 1.05em）。',
      aiMarkdownTableFontSize: 'AI Markdown 中表格的字型大小（如 0.9em）。',
      sidebarBg: '左側邊欄專用的背景色。',
      sidebarBtnColor: '側邊欄按鈕的圖示/文字色彩（預設狀態）。',
      sidebarBtnHoverBg: '暫留在側邊欄按鈕上時的背景色。',
      sidebarBtnHoverColor: '暫留在側邊欄按鈕上時的圖示/文字色彩。',
      sidebarBtnActiveBg: '目前選取的側邊欄按鈕的背景色。',
      tabBg: '非作用中索引標籤的背景色。',
      tabText: '非作用中索引標籤的文字色彩。',
      tabActiveText: '目前選取索引標籤的文字色彩。',
      tabCloseBg: '索引標籤關閉按鈕的色彩。',
      tabCloseHoverBg: '索引標籤關閉按鈕暫留時的色彩。',
      tabWatchingText: '正被 AI 監看的索引標籤的文字色彩。',
      tabWatchingBg: '索引標籤中 AI 監看圖示的背景/填色。',
      tabWatchingIcon: 'AI 監看圖示的主要光暈色彩。',
      tabWatchingIconGlow: 'AI 監看圖示漸層的次要光暈色彩。',
      tabConnectingBg: '工作階段連線過程中索引標籤的背景色。',
      tabConnectingText: '工作階段連線過程中索引標籤的文字色彩（也用於窗格覆疊文字）。',
      paneConnectingBg: '工作階段連線過程中顯示的窗格覆疊背景色。',
      contextMenuBg: '右鍵選單的背景色。',
      contextMenuBorder: '右鍵選單的框線。',
      contextMenuText: '右鍵選單的文字色彩。',
      contextMenuHoverBg: '選單項目暫留時的背景色。',
      hiddenItemBg: '隱藏項目（偵錯/管理）的專用背景色。',
      hiddenItemBgHover: '隱藏項目暫留時的背景色。',
      treeMetaColor: '清單檢視中中繼資料的色彩（如檔案大小）。',
      iconFolder: '主機樹狀結構中資料夾圖示的色彩。',
      iconHost: '主機樹狀結構中主機/連線圖示的色彩。',
      terminalPromptDefault: '終端機提示字元標記區塊的預設色彩。',
      terminalPromptActive: '被判定為命令輸入時提示字元標記區塊的作用中色彩。',
      terminalLetterboxBg: '固定寬度（釘選）終端機旁未使用區域的背景色調。',
      paneColor1: 'AI 交談對話配色 6 色中的第 1 色（監看中的終端機索引標籤、對話索引標籤與標題列標記）。',
      paneColor2: 'AI 交談對話配色 6 色中的第 2 色（監看中的終端機索引標籤、對話索引標籤與標題列標記）。',
      paneColor3: 'AI 交談對話配色 6 色中的第 3 色（監看中的終端機索引標籤、對話索引標籤與標題列標記）。',
      paneColor4: 'AI 交談對話配色 6 色中的第 4 色（監看中的終端機索引標籤、對話索引標籤與標題列標記）。',
      paneColor5: 'AI 交談對話配色 6 色中的第 5 色（監看中的終端機索引標籤、對話索引標籤與標題列標記）。',
      paneColor6: 'AI 交談對話配色 6 色中的第 6 色（監看中的終端機索引標籤、對話索引標籤與標題列標記）。',
      searchHighlightBg: '包含搜尋相符項目的行的背景色調。',
      searchHighlightCurrentBg: '目前聚焦的搜尋相符行的背景色。',
      searchHighlightCurrentBorder: '目前聚焦的相符行的外框色彩。',
      searchHighlightMarkBg: '行內相符文字範圍的背景醒目提示色。',
      searchHighlightMarkSolid: '目前聚焦行中相符文字範圍的實色背景。',
      searchHighlightMarkText: '醒目提示相符範圍的文字色彩。',
      modalOverlayBg: '強制回應對話方塊的背景變暗色彩。',
      modalShadow: '強制回應對話方塊陰影的色彩（rgba）。陰影幾何形狀固定為 0 4px 16px。',
      modalBorderWarning: '警告對話方塊的框線色彩（同時套用於對話方塊容器與標題分隔線）。',
      modalHeaderWarningBg: '警告對話方塊的標題列背景色。',
      modalHeaderWarningText: '警告對話方塊的標題列文字色彩。',
      modalBorderError: '錯誤對話方塊的框線色彩（同時套用於對話方塊容器與標題分隔線）。',
      modalHeaderErrorBg: '錯誤對話方塊的標題列背景色。',
      modalHeaderErrorText: '錯誤對話方塊的標題列文字色彩。',
      modalBorderSuccess: '成功對話方塊的框線色彩（同時套用於對話方塊容器與標題分隔線）。',
      modalHeaderSuccessBg: '成功對話方塊的標題列背景色。',
      modalHeaderSuccessText: '成功對話方塊的標題列文字色彩。',
      modalHeaderInfoBg: '資訊對話方塊的標題列背景色。',
      modalHeaderInfoBorder: '資訊對話方塊的標題列框線。',
      modalHeaderInfoText: '資訊對話方塊的標題列文字色彩。',
      updateNotificationBg: '更新通知橫幅的背景色。',
      updateNotificationBorder: '更新通知橫幅底部框線的色彩。',
      updateNotificationText: '更新通知橫幅的文字色彩。',
      updateNotificationAccent: '更新通知中版本醒目提示與圖示的強調色。',
      updateNotificationBtnBg: '更新通知中下載按鈕的背景色。',
      updateNotificationBtnText: '更新通知中下載按鈕的文字色彩。',
      updateNotificationBtnBorder: '更新通知中下載按鈕的框線色彩。',
      updateNotificationBtnHover: '更新通知中下載按鈕暫留時的背景色。',
      providerGemini1: 'Gemini 圖示漸層的第一個色標（起始色）。',
      providerGemini2: 'Gemini 圖示漸層的中間色標。',
      providerGemini3: 'Gemini 圖示漸層的最後一個色標（結束色）。',
      providerOpenai: 'OpenAI 圖示背景的品牌色。',
      providerAnthropic: 'Anthropic 圖示背景的品牌色。',
      providerVertexAi: 'Vertex AI 圖示的品牌色。',
      glowAccent: '作用中窗格外框與作用中側邊欄圖示的霓虹光暈色（建議使用 rgba）。',
      glowAccentStrong: '用於強調元素的較強霓虹光暈變化色。',
      glowBlur: '霓虹光暈的模糊半徑（如 10px）。數值越大光暈擴散越遠。',
      glassBg: '毛玻璃效果表面（對話方塊、側邊欄）的半透明背景。建議使用 alpha 約 0.7 的 rgba。',
      glassBlur: '毛玻璃表面的 backdrop-filter 模糊強度（如 14px）。',
      glassBorder: '疊加在毛玻璃表面上的淡框線色彩（低 alpha 的 rgba）。',
      iconStrokeWidth: '線條樣式 SVG 圖示的預設筆畫寬度（如 1.5）。',
    },
    title: '自訂佈景主題建立工具',
    cancel: '取消',
    themeName: '佈景主題名稱',
    themeNamePlaceholder: '例如 My Dark Blue',
    baseTheme: '基底佈景主題',
    saveTheme: '儲存佈景主題',
    saving: '正在儲存...',
    terminalSectionTitle: '終端機',
    terminalSectionDesc: 'xterm.js 終端機色彩',
    // 驗證
    errorEmptyName: '請輸入佈景主題名稱。',
    errorNoAlphanumeric: '佈景主題名稱必須至少包含一個英數字元。',
    errorProtectedName: '無法使用「dark」、「medium」或「light」作為佈景主題名稱。',
    errorSaveFailed: '無法儲存佈景主題。',
    // 區段標題與說明
    sectionBackgroundsTitle: '背景與文字',
    sectionBackgroundsDesc: '主要背景與文字色彩',
    sectionBordersTitle: '邊框與強調',
    sectionBordersDesc: '邊框色彩與強調／醒目色彩',
    sectionInputsTitle: '輸入欄位、按鈕與停留',
    sectionInputsDesc: '輸入欄位、按鈕與停留狀態色彩',
    sectionStatusTitle: '狀態與訊號',
    sectionStatusDesc: '成功、錯誤、警告與危險指示色彩',
    sectionAiChatTitle: 'AI 聊天',
    sectionAiChatDesc: 'AI 聊天訊息與程式碼區塊的色彩',
    sectionUiTitle: 'UI 專屬元件',
    sectionUiDesc: '側邊欄、分頁、操作功能表、圖示及其他 UI 元素',
    sectionProvidersTitle: 'AI 供應商',
    sectionProvidersDesc:
      'AI 供應商圖示所使用的品牌色彩（Gemini、OpenAI、Anthropic、Vertex AI）',
    sectionSearchTitle: '搜尋與醒目顯示',
    sectionSearchDesc: '搜尋結果的醒目顯示色彩',
    sectionOverlaysTitle: '覆蓋層與強制回應視窗',
    sectionOverlaysDesc: '強制回應對話方塊、覆蓋層與通知橫幅',
    sectionEffectsTitle: '未來感效果',
    sectionEffectsDesc:
      '套用於使用中窗格、側邊欄與強制回應視窗的霓虹光暈與玻璃擬態效果',
    // 終端機色彩列
    terminalForeground: '前景',
    terminalForegroundDesc: '終端機內的預設文字色彩',
    terminalBackground: '背景（使用中）',
    terminalBackgroundDesc: '使用中／聚焦終端機的背景',
    terminalBackgroundInactive: '背景（非使用中）',
    terminalBackgroundInactiveDesc: '非使用中／未聚焦終端機的背景',
    terminalPaneBackground: '窗格背景',
    terminalPaneBackgroundDesc: '終端機周圍空間的色彩',
  },
};

...
export const help = {
  ...
  hostTree: {
    ...
    netboxPlacement:
      '<0>按 IP 网段把主机归入文件夹:</0> 当同步设置为读取前缀时,每个 NetBox 文件夹都会记住登记在它名下的 IP 网段。在新建会话表单中输入地址,HoTTY 会指出覆盖该地址的文件夹并预先选中它。要整理已有的主机,右键点击某个文件夹(或树的空白处)并选择 <1>按 IP 网段归类…</1>:列表会明确显示哪些会移动、哪些已经正确、哪些没有任何文件夹或有多个文件夹同时认领。在你按下按钮之前不会移动任何东西,取消勾选的项目保持原位。当两个文件夹认领同一网段时,HoTTY 不会替你选择,而是原样保留并加以说明。',
    sshKeys:

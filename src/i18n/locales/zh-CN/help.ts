...
export const help = {
  ...
  logging: {
    ...
    aiChatLogging:
      '<0>AI 聊天日志：</0>启用日志记录后，每个 AI 聊天对话都会追加到同一文件夹中的 Markdown 文件（<1>...-AICHAT-....md</1>）并显示在日志查看器中。附加的图片会被记录但不会保存。开始新聊天、关闭标签页或切换 AI 提供商都会新建一个文件。您输入的文本会原样保存，因此请勿在聊天中输入凭据。',
    logViewer:
      '<0>日志查看器：</0>点击标签栏中的 <1>日志查看器</1> 按钮可打开专用的日志浏览窗格。它会列出所有已保存的日志文件，让您无需离开 HoTTY 即可打开和搜索它们。',
    fileTypes:
      '<0>文件类型：</0>会话日志以纯文本打开。AI 聊天记录（<1>.md</1>）会以排版后的形式打开——标题、表格和代码块与 AI 聊天窗格中看到的完全一致。Ping 监视器日志（<2>.csv</2>）会以表格形式打开。搜索栏中的按钮可将任一视图切回原始文件，搜索在所有视图中的用法相同。',
    pingCsv:
      '<0>Ping 监视器 CSV：</0>Ping 监视器会将结果保存到 <1>设置 → 常规</1> 中设定的同一个日志文件夹，因此其文件会与会话日志一起显示在日志查看器中。在设定该文件夹之前，<2>CSV 日志记录</2> 复选框不可用。',
    search:
  ...
  interfaceTraffic: {
    summary: '接口流量（SNMP）',
    intro:
      '通过 <0><1></1></0>（功能）→ <2>“Interface Traffic”</2> 打开接口流量面板。它通过 SNMP 轮询网络设备，在终端旁实时显示每个接口的吞吐量。',
    connect:
      '<0>连接：</0> 输入设备主机名或 IP 地址，并选择 SNMP 版本。<1>v2c</1> 使用团体字符串；<2>v3</2> 需要用户名和安全级别。',
    discover:
      '<0>列出接口：</0> 测试连接并列出设备上的接口。随后点击<1>开始</1>启动轮询——速率从第二次轮询开始显示，因为第一次用于建立基准值。',
    columns:
      '<0>列：</0> 入/出 <1>bps</1> 和 <2>pps</2>、收发错误与丢弃计数，以及每个接口的名称、描述、状态和速率。点击列标题可排序，可按名称或描述过滤，勾选<3>仅 Up</3>可隐藏已断开的接口。',
    v3:
      '<0>SNMP v3 安全：</0> 支持 <1>noAuthNoPriv</1>、<2>authNoPriv</2>（MD5/SHA 认证）和 <3>authPriv</3>（增加 DES/AES 加密），并可选填上下文名称。noAuthNoPriv 会以未认证的明文方式发送全部内容。',
    counters:
      '<0>64 位计数器：</0> 当设备提供 <1>ifXTable</1> 时使用。仅有 32 位计数器的设备同样可用，但面板会给出提示——在 1 Gbps 满负载链路上，32 位计数器约 34 秒即会回绕。',
    remember:
      '<0>保存的设置：</0> 每个面板保留各自的设备设置。勾选<1>记住这些连接设置</1>可一并保存凭据——保存前会使用 Windows DPAPI 加密。不勾选则每次都需重新输入。',
    enable:
      '<0>启用 / 禁用：</0> 可在<1>设置 → 功能</1>中关闭该面板。',
  },
  ...
  aiFeatures: {
    ...
    aiChatHeading: 'AI 聊天',
    aiChatIntro:
      '点击标签栏中的 <0><1></1></0>（功能）→ <2>“AI 聊天”</2> 打开 AI 聊天窗格。在其中，顶部的标签条可让您保持多个并行对话 — 使用 <3>+</3> 开始一个新标签页。输入您的问题并按 <4>Ctrl + Enter</4> 发送。',
    linkedTerminal:
      '<0>监视中的终端：</0>在某个终端上启动 AI 监视会将其添加到当前活动的 AI 聊天标签页 — 在多个终端上开启后，一个对话即可同时监视它们。每个监视中的终端会显示为输入框旁边的一个标记；点击标记可跳转到该终端，点击其 × 可停止监视，点击标记旁的 + 可添加另一个（已断开的终端会变灰，并在重新连接时自动重新链接）。当监视多个终端时，AI 会将每条命令发送到正确的终端；否则将在您最近使用的终端上执行。若需拥有各自终端集合的独立对话，请使用标签栏中的 +。',
    streamWatchdog:
    ...
    answerLanguage:
      '<0>回答语言：</0> AI 默认使用应用的显示语言回答，因此切换<1>设置 → 常规 → 显示语言</1>也会切换 AI 的回答语言。若要固定为其他语言，请打开 AI 聊天窗格顶部的<2>设置</2>并选择语言 — <3>Auto</3> 跟随界面语言，并标注当前实际使用的语言。该选择在所有窗口的所有会话之间共享（包括正在进行的会话），并从你的下一条消息开始生效。命令、终端输出、文件路径和标识符始终不会被翻译。',
    ...
    safetyBody:
      '每条命令如何判定是否自动执行由三个层级决定，可在 <0>设置 → AI → 命令执行</0> 中配置。<1>黑名单</1> 首先检查 — 匹配项永不自动运行（仍提供手动运行）。两个列表之上还有两道无法编辑掉的底线：包含重定向、串联或命令替换的命令绝不自动运行，可能将数据发送到本机之外的命令（curl、wget、nc、nmap 等）同样如此 — 无论选择哪种策略，它们始终会询问。<2>白名单</2> 会自动运行明显的只读命令。介于两者之间的任何命令都会发送给 <3>AI</3>，由它判断是否更改配置；只有被判定为只读且置信度足够的命令才会自动运行，其他一切都会询问。两个列表都完全可编辑：单个单词匹配基本命令（例如 <4>docker</4>），带空格的条目作为子字符串匹配（例如 <5>rm -rf</5>、<6>git push</6>）；每个列表都有一个 <7>恢复默认</7> 按钮。在同一处选择策略（静态 / AI / <8>混合</8>，默认）和 AI 置信度阈值。每个执行块都带有一行彩色判定 — 安全 · 白名单、安全 · AI、待查 · AI、已阻止或需要确认 — 点击即可展开完整原因，因此常规的安全执行不会喧宾夺主，而判定依据始终只有一次点击之遥。',
    personasHeading: '角色',
    personasBody:
    ...
    connectHeading: 'AI 打开的终端',
    connectBody:
      '在排查需要时，AI 可以请求 HoTTY 为其打开终端：<0>本机 Shell</0>（从您的电脑运行 ping、tracert 或 nslookup），或通过 CDP/LLDP 发现的<1>邻居设备的 SSH/Telnet 会话</1>。请求会以卡片形式出现在聊天中，显示目标、登录名以及凭据来源，您可在此批准或拒绝。AI 打开的会话没有标签页 — 其输出为 AI 捕获并显示在对话中 — 并以芯片形式显示在受监视终端旁边，您可以在那里将其作为真正的标签页打开或断开。它们按对话限制数量，空闲时自动关闭，并随对话关闭而关闭。可在<2>设置 → AI</2>中配置策略、Shell、上限和空闲时间；沿用受监视终端的密码默认关闭且总会先询问。SSH 主机密钥检查保持不变：首次连接未知设备仍会向您确认。',
  },
  ...
  themes: {
    ...
    language:
      '<0>显示语言：</0>在 <1>设置 → 常规 → 显示语言</1> 中切换界面语言。HoTTY 支持 English、日本語、简体中文、繁體中文、한국어、Русский、Español、Français，更改即时生效。（AI 回复语言默认跟随此设置，也可在 AI 聊天面板的设置中单独指定。）',
  },
};

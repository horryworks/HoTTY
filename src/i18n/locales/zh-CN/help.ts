...
export const help = {
  ...
  hostTree: {
    ...
    sshKeys:
      '<0>SSH 密钥:</0> <1>设置, SSH 密钥</1> 可生成密钥对,并管理 <2>.ssh</2> 文件夹中已有的密钥。除非设备另有要求,请选择 Ed25519。密钥以普通 OpenSSH 文件写入,因此 PuTTY、WinSCP 和 <3>ssh</3> 命令同样可以读取;每个新密钥都会被锁定,只有你的账户能够打开。每一行都可以复制公钥、保存为文件,或显示需要粘贴到服务器 <4>authorized_keys</4> 中的那一行。这里只能删除 HoTTY 创建的密钥。连接表单中的密钥选择框列出同样的密钥,并且可以不离开表单就生成新的密钥。',
  },

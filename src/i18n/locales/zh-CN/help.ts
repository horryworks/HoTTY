...
export const help = {
  ...
  gettingStarted: {
    ...
    doubleClick:
      '<0>双击：</0>双击树中的主机即可立即连接。双击文件夹，或点击其左侧的箭头，即可展开和折叠；单击只会选中它。',
    hostFilter:
      '<0>筛选:</0> 在主机树上方的输入框中键入内容，即可按文件夹名、主机名或地址缩小范围。<1>Ctrl + F</1> 可跳转到该输入框。Enter 选中第一个匹配项，Escape 清空输入。折叠文件夹内的匹配项也会显示出来，清除筛选后文件夹的展开状态会恢复原样。',
    ...
    updateNotifications:
      '<0>更新通知：</0>启动时 HoTTY 会检查 GitHub 发布源，有更新版本时显示可关闭的通知。预发布版本也包含在内，因此使用 Beta 版时，既会收到更新的 Beta 版通知，也会收到取代它的稳定版通知。通知会打开「设置 → 版本」，更新在那里应用。',
    versionSwitching:
      '<0>切换版本：</0>「设置 → 版本」列出所有已发布的版本及其发布说明，并可安装你选择的版本 — 既可升级到更新的版本，也可回退到较早的版本。HoTTY 自行下载安装程序，用 GitHub 为该文件发布的校验和进行核对，并在 HoTTY 退出时启动它，以确保替换文件期间没有程序在运行。回退后主机、主题和已保存的凭据都会保留；新版本新增的设置只是被忽略，不会删除。GitHub 未发布校验和的版本会显示在列表中，但无法安装。',
    connectionStatus:
  ...
  hostTree: {
    ...
    netbox:
      '<0>从 NetBox 同步文件夹：</0>在<1>设置 → NetBox</1>中填入 NetBox 地址和 API 令牌，HoTTY 会把其 Regions 与 Sites 映射到树末尾名为 <2>NetBox</2> 的文件夹中。只读取，绝不写回。点击主机树工具栏的循环箭头按钮即可同步，也可以让它在 HoTTY 每次启动时自动执行一次。站点文件夹命名为“代码 站点名称”；代码保存在哪个 NetBox 字段由你选择，因为各组织的做法不同。',
    netboxOwnership:
      '<0>同步管理的范围：</0>NetBox 文件夹内部的名称与排列由 NetBox 决定，每次同步都会重写，因此这些文件夹无法在此重命名。你在站点文件夹中添加的主机绝不会被改动。任何内容都不会被删除：从 NetBox 中消失的文件夹会被标记为“NetBox 中已不存在”并保留，因为里面可能有你添加的主机。把文件夹拖出 NetBox 文件夹后，同步就不再移动它，只继续更新名称。',
    netboxPlacement:
      '<0>按 IP 网段把主机归入文件夹:</0> 当同步设置为读取前缀时,每个 NetBox 文件夹都会记住登记在它名下的 IP 网段。在新建会话表单中输入地址,HoTTY 会指出覆盖该地址的文件夹并预先选中它。要整理已有的主机,右键点击某个文件夹(或树的空白处)并选择 <1>按 IP 网段归类…</1>:列表会明确显示哪些会移动、哪些已经正确、哪些没有任何文件夹或有多个文件夹同时认领。在你按下按钮之前不会移动任何东西,取消勾选的项目保持原位。当两个文件夹认领同一网段时,HoTTY 不会替你选择,而是原样保留并加以说明。 也可以只移动一台主机：右键该主机会给出它将进入的文件夹名称，或者说明为什么不会移动 — 没有文件夹覆盖该地址、有多个文件夹覆盖，或者它根本不是地址。整棵树的排序现在在「设置、NetBox」里也有一个按钮，那里才是大家会去找的地方。',
    sshKeys:
      '<0>SSH 密钥:</0> <1>设置, SSH 密钥</1> 可生成密钥对,并管理 <2>.ssh</2> 文件夹中已有的密钥。除非设备另有要求,请选择 Ed25519。密钥以普通 OpenSSH 文件写入,因此 PuTTY、WinSCP 和 <3>ssh</3> 命令同样可以读取;每个新密钥都会被锁定,只有你的账户能够打开。每一行都可以复制公钥、保存为文件,或显示需要粘贴到服务器 <4>authorized_keys</4> 中的那一行。这里只能删除 HoTTY 创建的密钥。连接表单和主机树的添加主机表单中都有同样的密钥选择框,两者都列出同样的密钥,并且可以不离开表单就生成新的密钥。',
  },
  ...
  aiFeatures: {
    ...
    ownWindowHeading: "在独立窗口中使用 AI Chat",
    ownWindowBody:
      "AI Chat 标题栏中垃圾桶旁边的按钮可以把整个面板移到独立窗口：对话、正在观察的终端、以及 AI 打开的终端都会一起过去，HoTTY 窗口就能完全留给终端。同一个按钮可以移回。在该窗口中，图钉按钮让它保持在其他应用之上，位置和大小会被记住。一个对话只存在于一处，因此在接收回复、执行命令或等待你确认期间按钮会变灰。关闭该窗口会结束其中的对话；有内容会丢失时会先询问。终端标签的右键菜单中也有「在 AI Chat 窗口中查看」。",

...
export const help = {
  ...
  gettingStarted: {
    ...
    updateNotifications:
      '<0>Notifications de mise à jour :</0> Au démarrage, HoTTY consulte le flux des versions GitHub et affiche une notification que vous pouvez fermer lorsqu\'une version plus récente est disponible. Les préversions sont incluses : une bêta est donc informée des bêtas plus récentes comme de la version stable qui la remplace. La notification ouvre Paramètres → Versions, où la mise à jour est appliquée.',
    versionSwitching:
      '<0>Changer de version :</0> Paramètres → Versions répertorie toutes les versions publiées avec leurs notes et installe celle que vous choisissez — vers une version plus récente ou de retour vers une plus ancienne. HoTTY télécharge lui-même l\'installateur, le compare à la somme de contrôle publiée par GitHub pour ce fichier, puis le lance au moment où HoTTY se ferme, afin que rien ne tourne pendant le remplacement des fichiers. Revenir en arrière conserve vos hôtes, thèmes et identifiants enregistrés ; les paramètres ajoutés par une version plus récente sont ignorés, pas supprimés. Une version pour laquelle GitHub n\'a publié aucune somme de contrôle est listée mais ne peut pas être installée.',
    connectionStatus:
  ...
  hostTree: {
    ...
    netbox:
      "<0>Synchroniser des dossiers depuis NetBox :</0> Dans <1>Paramètres, NetBox</1>, saisissez l'adresse de votre NetBox et un jeton d'API : HoTTY reproduit ses Regions et Sites dans un dossier nommé <2>NetBox</2> au bas de l'arborescence. Il se contente de lire ; rien n'est jamais réécrit. Appuyez sur le bouton à flèches circulaires dans la barre d'outils de l'arborescence pour synchroniser, ou laissez-le s'exécuter une fois à chaque démarrage. Les dossiers de site sont nommés « code nom du site » ; c'est à vous de choisir le champ NetBox qui contient le code, car il varie selon l'organisation.",
    netboxOwnership:
      "<0>Ce que gère la synchronisation :</0> Les noms et la disposition à l'intérieur du dossier NetBox appartiennent à NetBox et sont réécrits à chaque synchronisation ; ces dossiers ne peuvent donc pas être renommés ici. Les hôtes que vous ajoutez dans un dossier de site ne sont jamais touchés. Rien n'est jamais supprimé : un dossier disparu de NetBox est marqué « absent de NetBox » et conservé, car il peut contenir des hôtes que vous avez ajoutés. Sortez un dossier du dossier NetBox et la synchronisation cesse de le déplacer — elle se contente d'en tenir le nom à jour.",
    netboxPlacement:
      "<0>Classer les hôtes par plage d'adresses IP :</0> Lorsque la synchronisation lit les préfixes, chaque dossier NetBox mémorise les plages IP qui lui sont associées. Saisissez une adresse dans le formulaire Nouvelle session et HoTTY nomme le dossier qui la contient et le présélectionne. Pour trier les hôtes déjà présents, faites un clic droit sur un dossier (ou sur le fond de l'arborescence) et choisissez <1>Trier par plage d'adresses IP…</1> : la liste indique exactement ce qui serait déplacé, ce qui est déjà au bon endroit, et ce qu'aucun dossier — ou plusieurs dossiers — revendique. Rien ne bouge tant que vous n'avez pas appuyé sur le bouton, et tout ce que vous décochez reste en place. Quand deux dossiers revendiquent la même plage, HoTTY ne tranche pas : il laisse ces hôtes tels quels et le signale. Un hôte peut aussi être déplacé seul : son menu contextuel nomme le dossier de destination, ou explique pourquoi il ne bougera pas — aucun dossier ne couvre l'adresse, plusieurs la couvrent, ou ce n'est pas une adresse. Le tri de tout l'arbre dispose désormais aussi d'un bouton dans Paramètres, NetBox, là où on le cherche.",
    sshKeys:
      "<0>Clés SSH :</0> <1>Paramètres, Clés SSH</1> génère une paire de clés et gère celles déjà présentes dans votre dossier <2>.ssh</2>. Choisissez Ed25519 sauf si un équipement impose autre chose. Les clés sont écrites comme des fichiers OpenSSH ordinaires, donc PuTTY, WinSCP et la commande <3>ssh</3> les lisent aussi, et chaque nouvelle clé est verrouillée pour que seul votre compte puisse l'ouvrir. Chaque ligne permet de copier la clé publique, de l'enregistrer dans un fichier ou d'afficher la ligne à coller dans <4>authorized_keys</4> sur le serveur. Seules les clés créées par HoTTY peuvent être supprimées ici. La même ligne de clé apparaît dans le formulaire de connexion et dans le formulaire d'ajout d'hôte de l'arbre des hôtes : tous deux affichent cette liste et peuvent en générer une nouvelle sans quitter le formulaire.",
  },
  ...
  aiQuickStart: {
    ...
    step2:
      '<0>Connectez-vous :</0> le formulaire de connexion du fournisseur sélectionné apparaît juste sous le sélecteur de fournisseur dans <1>Paramètres → IA</1> — saisissez-y vos identifiants.',
    step3:
  ...
  aiFeatures: {
    ...
    ownWindowHeading: "AI Chat dans sa propre fenêtre",
    ownWindowBody:
      "Le bouton à côté de la corbeille, dans l'en-tête d'AI Chat, déplace tout le panneau — chaque conversation, ses terminaux suivis et ceux ouverts par l'IA — vers une fenêtre dédiée, laissant toute la fenêtre HoTTY au terminal. Le même bouton le ramène. Dans cette fenêtre, un bouton d'épingle la garde au premier plan, et sa position et sa taille sont mémorisées. Une conversation n'existe qu'à un seul endroit : le bouton est donc grisé pendant la réception d'une réponse, l'exécution d'une commande ou l'attente d'une confirmation. Fermer la fenêtre met fin à ses conversations et demande confirmation s'il y a quelque chose à perdre. Le menu contextuel d'un onglet de terminal propose aussi « Suivre dans la fenêtre AI Chat ».",
    ...
    interactiveBody:
      'Lorsque l\'IA suggère une commande, elle peut l\'envoyer directement à votre session de terminal pour exécution. Vous verrez la commande avant son exécution, vous gardez donc le contrôle. Lorsqu\'une commande suggérée attend votre confirmation, vous pouvez l\'exécuter ou cliquer sur « Ne pas exécuter » pour la refuser ; le refus est signalé à l\'IA, qui peut alors proposer une autre approche.',
    ...
    safetyBody:
      'La façon dont chaque commande est jugée pour l\'exécution automatique est décidée par trois couches, configurables dans <0>Paramètres → IA → Exécution de commandes</0>. La <1>liste noire</1> est vérifiée en premier — une correspondance ne s\'exécute jamais automatiquement (une exécution manuelle reste proposée). Deux planchers non modifiables se placent au-dessus des deux listes : une commande qui redirige, enchaîne ou substitue ne s\'exécute jamais automatiquement, pas plus qu\'une commande capable d\'envoyer des données hors de cette machine (curl, wget, nc, nmap et similaires) ; celles-ci demandent toujours, quelle que soit la stratégie choisie. La <2>liste blanche</2> exécute automatiquement les commandes manifestement en lecture seule. Tout ce qui se situe entre les deux est envoyé à l\'<3>IA</3>, qui juge si cela modifie la configuration ; seules les commandes jugées en lecture seule avec une confiance suffisante s\'exécutent automatiquement, tout le reste demande confirmation. Les deux listes sont entièrement modifiables : un mot unique correspond à une commande de base (ex. <4>docker</4>), et une entrée avec des espaces correspond comme sous-chaîne (ex. <5>rm -rf</5>, <6>git push</6>) ; chaque liste possède un bouton <7>Réinitialiser par défaut</7>. Choisissez la stratégie (Statique / IA / <8>Hybride</8>, par défaut) et le seuil de confiance de l\'IA au même endroit. Chaque bloc d\'exécution porte une ligne de verdict en couleur — Sûr · liste blanche, Sûr · IA, À vérifier · IA, Bloqué ou Confirmation requise — et un clic déploie la raison complète, afin qu\'une exécution sûre de routine reste discrète tout en gardant la décision à un clic.',
    ...
    connectHeading: "Terminaux ouverts par l'IA",
    connectBody:
      "Lorsqu'une investigation le nécessite, l'IA peut demander à HoTTY de lui ouvrir un terminal : un <0>shell du PC</0> (pour lancer ping, tracert ou nslookup depuis votre ordinateur) ou une <1>session SSH/Telnet vers un voisin</1> découvert avec CDP/LLDP. La demande apparaît sous forme de carte dans la discussion, avec la cible, l'identifiant et la provenance des identifiants ; vous l'approuvez ou la refusez à cet endroit. Les sessions ouvertes par l'IA n'ont pas d'onglet — leur sortie est capturée pour l'IA et apparaît dans la conversation — et s'affichent sous forme de puces à côté des terminaux surveillés, où vous pouvez en ouvrir une comme véritable onglet ou la déconnecter. Elles sont limitées par conversation, fermées automatiquement lorsqu'elles sont inactives et fermées avec la conversation. Configurez la politique, le shell, la limite et le délai d'inactivité dans <2>Paramètres → IA</2> ; la réutilisation du mot de passe d'un terminal surveillé est désactivée par défaut et demande toujours confirmation. La vérification de la clé d'hôte SSH est inchangée : une première connexion à un équipement inconnu vous demande toujours confirmation.",
  },
  ...
  aiSetup: {
    ...
    whereHeading: 'Où saisir mes identifiants ?',
    whereBody1:
      'Tout se trouve dans <0>Paramètres → IA</0> : choisissez-y un fournisseur, et le formulaire de connexion de ce fournisseur apparaît juste en dessous, avec votre état d\'authentification et un bouton de déconnexion.',
    whereBody2:
      'Le <0>panneau de chat IA</0> lui-même n\'a aucun champ d\'identifiants — tant que vous n\'êtes pas connecté, il affiche un bouton <1>Ouvrir les Paramètres</1> qui mène directement à l\'onglet IA. Vous ne vous connectez qu\'une seule fois ; HoTTY se ré-authentifie automatiquement aux lancements suivants, et la déconnexion se fait au même endroit.',
    geminiHeading: 'Google AI Studio (Gemini) — Se connecter avec Google (OAuth2)',
    geminiIntro:
      'Ce fournisseur se connecte avec votre compte Google via OAuth2 — une <0>clé API AI Studio ne peut pas être utilisée</0>. Créez une seule fois votre propre client OAuth (gratuit) :',
    geminiStep1:
      'Dans <0>Google Cloud Console</0> (console.cloud.google.com), créez ou sélectionnez un projet, puis activez la <1>Generative Language API</1> (API et services → Bibliothèque).',
    geminiStep2:
      'Configurez l\'<0>écran de consentement OAuth</0> (API et services) : « Externe » convient. Tant que l\'application est en mode <1>Test</1>, ajoutez votre propre compte Google dans <2>Utilisateurs test</2> — sans cela, Google bloque la connexion.',
    geminiStep3:
      'Créez le client dans API et services → Identifiants → <0>Créer des identifiants → ID client OAuth</0>, type d\'application <1>Application de bureau</1>, puis copiez l\'<2>ID client</2> et le <3>secret client</3>. Aucune URI de redirection n\'est nécessaire — HoTTY écoute automatiquement sur un port localhost temporaire.',
    geminiStep4:
      'Dans HoTTY, ouvrez <0>Paramètres → IA</0>, sélectionnez <1>Google AI Studio (Gemini)</1> comme fournisseur, collez l\'ID client et le secret client dans le formulaire en dessous, puis cliquez sur <2>« Se connecter avec Google »</2>.',
    geminiStep5:
      'Votre navigateur ouvre la page de consentement de Google — approuvez l\'accès dans les 5 minutes. HoTTY reçoit la connexion localement et le chat est prêt à l\'emploi.',
    geminiNote:
    ...
    vertexStep1:
      'Dans Google Cloud Console, activez la <0>Vertex AI API</0> pour votre projet et vérifiez que votre compte (ou le compte de service) dispose du rôle <1>Utilisateur Vertex AI</1> (roles/aiplatform.user).',
    vertexStep2:
    ...
    vertexStep3:
      '<0>Compte de service (alternative) :</0> créez une clé JSON dans IAM et administration → Comptes de service et enregistrez-la localement ; dans le panneau de connexion, choisissez <1>Compte de service</1> et indiquez le chemin du fichier de clé.',
    vertexStep4:
      'Dans <0>Paramètres → IA</0>, saisissez votre <1>ID de projet Google Cloud</1>, sélectionnez une <2>Région</2>, choisissez le type d\'authentification, puis cliquez sur <3>Se connecter</3>.',
    anthropicHeading: 'Anthropic (Claude) — Clé API',
    anthropicExperimental: '( Expérimental)',
    anthropicStep1:
      'Créez un compte sur <0>console.anthropic.com</0> et ajoutez des crédits API — la facturation de l\'API est distincte d\'un abonnement Claude.ai.',
    anthropicStep2:
      'Créez une clé dans Paramètres → <0>API Keys</0> et copiez-la immédiatement — elle n\'est affichée qu\'une seule fois (elle commence par <1>sk-ant-</1>).',
    anthropicStep3:
      'Dans HoTTY, ouvrez <0>Paramètres → IA</0>, sélectionnez <1>Anthropic</1> comme fournisseur, collez la clé API et cliquez sur Se connecter.',
    anthropicNote:
    ...
    openaiStep1:
      'Créez un compte sur <0>platform.openai.com</0> et configurez la facturation ou les crédits — la facturation de l\'API est distincte de ChatGPT Plus.',
    openaiStep2:
      'Créez une clé dans <0>API keys</0> et copiez-la immédiatement — elle n\'est affichée qu\'une seule fois (elle commence par <1>sk-</1>).',
    openaiStep3:
      'Dans HoTTY, ouvrez <0>Paramètres → IA</0>, sélectionnez <1>OpenAI</1> comme fournisseur, collez la clé API et cliquez sur Se connecter.',
    openaiNote:
      'Ce fournisseur est expérimental et n\'a pas été entièrement testé. Certaines fonctionnalités (mode Surveillance, mode interactif, etc.) peuvent ne pas fonctionner comme prévu. Veuillez signaler tout problème rencontré.',
    troubleshootHeading: 'Si la connexion échoue',
    troubleshootGemini:
      '<0>Gemini :</0> une page « Accès bloqué » / « l\'application n\'a pas terminé la validation » signifie que votre compte Google ne figure pas dans les <1>Utilisateurs test</1> de l\'écran de consentement OAuth. Une tentative de connexion expire aussi au bout de 5 minutes — relancez-la simplement.',
    troubleshootVertex:
      '<0>Vertex AI :</0> les erreurs de permission (403) signifient généralement que la Vertex AI API n\'est pas activée ou que le rôle <1>Utilisateur Vertex AI</1> est manquant. Si l\'ADC a expiré, relancez <2>gcloud auth application-default login</2>.',
    troubleshootKeys:
      '<0>OpenAI / Anthropic :</0> 401 signifie que la clé est invalide ou révoquée ; 429 signifie généralement qu\'il n\'y a plus de crédits ou qu\'une limitation de débit s\'applique.',
    credentialsNote:

...
export const help = {
  ...
  gettingStarted: {
    ...
    updateNotifications:
      '<0>Update Notifications:</0> On startup HoTTY checks the GitHub releases feed and shows a dismissible notification when a newer version is available. Pre-releases are included, so a beta build is told about newer betas as well as the stable release that supersedes it. The notification opens Settings → Versions, where the update is applied.',
    versionSwitching:
      '<0>Switching Versions:</0> Settings → Versions lists every published release with its notes and installs the one you pick — forward to a newer build, or back to an older one. HoTTY downloads the installer itself, checks it against the checksum GitHub published for that asset, and starts it as HoTTY exits, so nothing is running while files are replaced. Going back keeps your hosts, themes and saved credentials; settings added by a newer version are ignored, not deleted. A release GitHub published no checksum for is listed but cannot be installed.',
    connectionStatus:
  ...
  aiFeatures: {
    ...
    safetyBody:
      'How each command is judged for auto-execution is decided by three layers, configurable in <0>Settings → AI → Command Execution</0>. The <1>Blacklist</1> is checked first — a match never auto-runs (a manual Run is still offered). Two floors sit above both lists and cannot be edited away: a command that redirects, chains or substitutes never auto-runs, and neither does one that can send data off this machine (curl, wget, nc, nmap and the like) — those always ask, whichever strategy you pick. The <2>Whitelist</2> auto-runs obvious read-only commands. Anything in between is sent to the <3>AI</3>, which judges whether it changes configuration; only commands judged read-only with enough confidence auto-run, everything else asks. Both lists are fully editable: a single word matches a base command (e.g. <4>docker</4>), and an entry with spaces matches as a substring (e.g. <5>rm -rf</5>, <6>git push</6>); each list has a <7>Reset to defaults</7> button. Pick the strategy (Static / AI / <8>Hybrid</8>, the default) and the AI confidence threshold in the same place. Every execute block carries a colour-coded verdict line — Safe · whitelisted, Safe · AI, Check · AI, Blocked, or Needs confirmation — and clicking it expands the full reason, so a routine safe run reads quietly while the decision stays one click away.',
    ...
    connectHeading: 'AI-opened terminals',
    connectBody:
      "When an investigation needs it, the AI can ask HoTTY to open a terminal for it: a <0>PC shell</0> (to run ping, tracert or nslookup from your computer) or an <1>SSH/Telnet session to a neighbour</1> it discovered with CDP/LLDP. The request appears as a card in the chat showing the target, the login name and where the credentials would come from; you approve or decline it there. AI-opened sessions have no tab — their output is captured for the AI and appears in the conversation — and they show up as chips beside the watched terminals, where you can open one as a real tab or disconnect it. They are limited per conversation, closed automatically when idle, and closed when the conversation is closed. Configure the policy, the shell, the limit and the idle timeout in <2>Settings → AI</2>; reusing a watched terminal's password is off by default and always asks first. The SSH host-key check is unchanged: a first connection to an unknown device still asks you.",
  },

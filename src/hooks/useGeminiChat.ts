...
export function useGeminiChat(options: UseGeminiChatOptions): UseGeminiChatReturn {
  ...
  const resolvePersonaPrompt = useCallback((expertiseLabel?: string): string => {
    ...
    return targetPrompt + " When you suggest shell/terminal commands that the user can run, always enclose them in a code block marked with ```execute for direct execution.";
  }, []);
